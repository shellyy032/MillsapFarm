...
<body class="bg-#teal-900 text-[#34495E] min-h-screen flex flex-col">
    ...
    <h1 class="text-center text-6xl font-extrabold text-yellow-600 mt-8 tracking-wider">ROLE & ACCESS CONTROL</h1>
                    ...
                    <tr class="border-t text-gray-700">
                        <td class="p-1">{{ $mod }}</td>
                            @foreach (['create','read','update','delete'] as $perm)
                                <td class="p-1">
                                    <input type="checkbox" name="permissions[{{ $mod }}][{{ $perm }}]" class="h-4 w-4" @if($permissions[$currentRole][$mod][$perm] ?? false) checked @endif>
                                </td>
                                @endforeach
                            </tr>
                            @endforeach
